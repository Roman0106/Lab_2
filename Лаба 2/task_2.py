...
class Library:
    ...
    def get_next_book_id(self):
        if not self.books:
            return 1  # Если книг нет, возвращаем 1
        else:
            return self.books[-1].id + 1  # Возвращаем id последней книги + 1

    def get_index_by_book_id(self, book_id):
        for index, book in enumerate(self.books):
            if book.id == book_id:
                return index  # Возвращаем индекс книги
        raise ValueError("Книги с запрашиваемым id не существует")  # Если книга не найдена
